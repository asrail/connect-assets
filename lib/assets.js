...
(function() {
  var BEFORE_DOT, ConnectAssets, EXPLICIT_PATH, REMOTE_PATH, Snockets, concatFilenamer, connectCache, crypto, cssCompilers, cssExts, exports, extend, fs, getExt, identityFilenamer, jsCompilers, libs, md5Filenamer, mkdirRecursive, parse, path, stripExt, timeEq,
    __slice = [].slice;
  ...
  module.exports = exports = function(options) {
    ...
    if ((_ref8 = options.jsBuildFilenamer) == null) {
      options.jsBuildFilenamer = concatFilenamer;
    }
    if ((_ref9 = options.buildsExpire) == null) {
      options.buildsExpire = false;
    }
    if ((_ref10 = options.detectChanges) == null) {
      options.detectChanges = process.env.NODE_ENV !== 'production';
    }
    if ((_ref11 = options.minifyBuilds) == null) {
      options.minifyBuilds = true;
    }
    if ((_ref12 = options.pathsOnly) == null) {
  ...
  ConnectAssets = (function() {
    ...
    ConnectAssets.prototype.compileJS = function(route) {
      var buildPath, builtFile, callback, chain, ext, filename, js, snocketsFlags, sourceDir, sourceFile, sourcePath, timestamp, _i, _j, _len, _len1, _ref, _ref1,
        _this = this;
      sourceDir = path.join(process.cwd(), this.options.src);
      if (this.options.buildDir) {
        buildPath = path.join(process.cwd(), this.options.buildDir);
        ...
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          ...
          if (fs.existsSync(sourceFile)) {
            ...
            filename = this.options.jsBuildFilenamer(route, timestamp);
            builtFile = "" + buildPath + "/" + filename;
            if (!this.options.detectChanges && fs.existsSync(builtFile)) {
              return ["/" + filename];
            }
          }
        }
        ...
        try {
          if (this.options.build) {
            ...
            callback = function(err, concatenation, changed) {
              var buildDir, cacheFlags;
              if (err) {
                throw err;
              }
              if (changed) {
                filename = _this.options.jsBuildFilenamer(route, timestamp);
            ...
            return this.cachedRoutePaths[route] = (function() {
              ...
              _results = [];
              for (_k = 0, _len2 = chain.length; _k < _len2; _k++) {
                _ref2 = chain[_k], filename = _ref2.filename, js = _ref2.js;
  ...
  exports.md5Filenamer = md5Filenamer = function(filename, code) {
    ...
    return concatFilenamer(filename, md5Hex);
  };

  exports.identityFilenamer = identityFilenamer = function(filename) {
    return filename;
  };

  exports.concatFilenamer = concatFilenamer = function(filename, suffix) {
    var ext;
    ext = path.extname(filename);
    return "" + (stripExt(filename)) + "-" + suffix + ext;
  };
